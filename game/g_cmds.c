...
void Cmd_SpawnChest_f(edict_t* ent) {
	if (!ent->client)
		return;

	edict_t *chest = G_Spawn();
	if (!chest) {
		gi.cprintf(ent, PRINT_HIGH, "Failed to spawn chest.\n");
		return;
	}
	VectorCopy(ent->s.origin, chest->s.origin);
	chest->classname = "item_chest";
	if (!chest->classname) {
		gi.cprintf(ent, PRINT_HIGH, "Failed to set classname.\n");
		G_FreeEdict(chest);
		return;
	}
	SP_item_chest(chest);
	gi.cprintf(ent, PRINT_HIGH, "Chest spawned succesfully.\n");
}

void Cmd_Init(void) {
	gi.AddCommandString("spawn_chest", Cmd_SpawnChest_f);
}
